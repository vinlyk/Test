...
import analyzer  # import once — do NOT reload inside tests
...
# ---------------------------------------------------------------------------
# Helper: run main() with patched dependencies
# ---------------------------------------------------------------------------

def run_cli(*argv, extra_patches=None):
    """
    Run analyzer.main() with mocked external dependencies.
    Returns (exit_code, stdout_str, stderr_str).
    """
    patches = [
        patch.dict("os.environ", {"ANTHROPIC_API_KEY": "test-key-xxx"}, clear=False),
...
class TestMissingApiKey:
    def test_missing_key_exits_2(self):
        with patch.dict("os.environ", {}, clear=True), \
             patch("sys.argv", ["analyzer.py", "https://youtu.be/dQw4w9WgXcQ"]), \
             patch("sys.stdout", new_callable=StringIO), \
    ...
    def test_missing_key_error_message(self):
        with patch.dict("os.environ", {}, clear=True), \
            ...
            try:
                ...
                pass
            assert "ANTHROPIC_API_KEY" in mock_err.getvalue()
...
class TestLanguageFlag:
    def test_language_passed_to_fetch(self):
        with patch.dict("os.environ", {"ANTHROPIC_API_KEY": "test-key"}), \
             patch("sys.argv", ["analyzer.py", "https://youtu.be/dQw4w9WgXcQ", "--language", "de"]), \
             patch("sys.stdout", new_callable=StringIO), \
             patch("sys.stderr", new_callable=StringIO), \
             patch("analyzer.extract_video_id", return_value="dQw4w9WgXcQ"), \
...
class TestTranscriptError:
    def test_transcript_error_exits_1(self):
        with patch.dict("os.environ", {"ANTHROPIC_API_KEY": "test-key"}), \
             patch("sys.argv", ["analyzer.py", "https://youtu.be/dQw4w9WgXcQ"]), \
             patch("sys.stdout", new_callable=StringIO), \
             patch("sys.stderr", new_callable=StringIO), \
    ...
    def test_transcript_error_message_in_stderr(self):
        with patch.dict("os.environ", {"ANTHROPIC_API_KEY": "test-key"}), \
             patch("sys.argv", ["analyzer.py", "https://youtu.be/dQw4w9WgXcQ"]), \
             patch("sys.stdout", new_callable=StringIO), \
             patch("sys.stderr", new_callable=StringIO) as mock_err, \
...
class TestTimestampsFlag:
    def test_timestamps_passed_to_clean(self):
        with patch.dict("os.environ", {"ANTHROPIC_API_KEY": "test-key"}), \
             patch("sys.argv", ["analyzer.py", "https://youtu.be/dQw4w9WgXcQ", "--timestamps"]), \
             patch("sys.stdout", new_callable=StringIO), \
             patch("sys.stderr", new_callable=StringIO), \
             patch("analyzer.extract_video_id", return_value="dQw4w9WgXcQ"), \
             patch("analyzer.fetch_transcript", return_value=_RAW_DATA), \
